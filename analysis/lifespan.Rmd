...
Throughout the lifespan and given the set-up of cages, flies got stuck to the food. The incidence of which correlates very strongly with age. Line 897 below is an exception to this, whereby flies, females especially, were suck to the food from quite early on. While it seems a lot of flies do travel to food to die, these flies while young were very much alive but just stuck.

Below I have censored the flies that were stuck, but included the counts of flies at each score that were on the food on the second y-axis. This helps visualise how much this line was susceptible to getting stuck.

```{r quick plot survcurv, fig.cap="Surv curv of Line 897 with food-stuck flies censored, but counts of which juxtaposed on seccond y-axis"}
...
If I include the flies on the food in the lifespan as deaths, the survival curve looks very different with females having a much lower median lifespan. 

```{r 897 food-stuck included, fig.cap="Surv curv of Line 897 with food-stuck included as deaths"}
data_include_stuck <-
  data_long %>% 
  separate(5, c("Sex", "Event")) %>% 
  mutate(Event = recode(Event, 
...
```{r line 287 plot prep, echo=F, hide=T, warning=F}

data_287 <- read.csv(files[1])
...
```{r quick plot 287 survcurv, fig.cap="Surv curv of Line 287 with food-stuck flies censored, but counts of which juxtaposed on seccond y-axis"}
...
```{r line 287 included, fig.cap="Surv curv of Line 287 with food-stuck included as deaths"}
...
Surv_included <- 
  ggplot(subset(Survival_table_2,Survival_table_2 > 0), aes(x=Time))+
  geom_line(aes(y=Survival,colour=as.factor(Sex)), size = 1.5)+
  ggtitle("Line 287 - Lifespan (including stuck)")+
  scale_color_manual(name="Group",
                     labels = c("Female", "Male"),
                     values = c("firebrick", "navy"))+
   scale_y_continuous("Proportion of survivors",
