...
```{r coxph analysis, warning=F, fig.cap="Corresponding regression coefficients"}
...
```{r survplot individual, warning = F, echo =F}
...
## Quick plot generation to compare lysing methods
```{r}

lyse <- read.csv("data/LyseCompare.csv")

lyse_clean <- 
   lyse %>% 
   separate(1, c("Method", "Sample"))

lyseComp <-
   ggplot(lyse_clean, aes(Method,CFU_fly)) +
...
p = plot_grid(lyseComp, Initial_dose_2, labels = "AUTO")
p

ggsave("output/dose_compare.pdf", p)
```
